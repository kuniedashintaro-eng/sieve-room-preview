...
export async function POST(request: Request) {
  ...
  const prompt = `This is a realistic room photo uploaded by the user. The first input image is the user's room and must be preserved. Do not clean, declutter, remove, replace, hide, rearrange, or simplify any existing objects, small items, furniture, shelves, toys, clothes, appliances, rugs, curtains, wall items, or personal belongings in the user's room photo unless the new furniture physically overlaps the specified placement area. Keep the user's original room structure, existing objects, perspective, lighting, shadows, scale, floor contact, camera angle, and atmosphere.

The second input image is only the visual reference for the selected SIEVE furniture. Use only the furniture body in the second image as the exact reference for shape, silhouette, seating capacity, arm/back position, legs, frame, cushions, upholstery texture, material, visible seams, color, and proportions. For the second input image only, ignore and do not reproduce any lifestyle props, people, hands, blankets, pillows that are not part of the product, trays, cups, rugs, plants, walls, floors, windows, lighting fixtures, tables, background rooms, labels, thumbnails, color swatches, or alternate color variations visible in the product reference. Prioritize only the selected furniture's main visible color and material from the reference image; do not mix in other color variants.

Place that exact selected SIEVE furniture naturally into the specified area of the user's room. ${seatCountInstruction} Do not change the room unnecessarily. Do not turn the furniture into a bed, mattress, different sofa type, different seating count, or a generic similar product. Use only the selected SIEVE product as the furniture reference. Product name: ${selectedProduct.name}. Product category: ${selectedProduct.category}. Product description: ${selectedProduct.description}. Placement instruction: ${placementInstruction}. Generate a realistic interior preview image for EC purchase consideration.`;
